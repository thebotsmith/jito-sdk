...
import (
	...
	auth "github.com/Prophet-Solutions/jito-sdk/pb/auth"
...
func NewAuthenticationService(
	...
	return &AuthenticationService{
		AuthService: auth.NewAuthServiceClient(grpcConn),
		GRPCCtx:     ctx,
		KeyPair:     keyPair,
...
func (as *AuthenticationService) AuthenticateAndRefresh(role auth.Role) error {
	...
	respChallenge, err := as.AuthService.GenerateAuthChallenge(as.GRPCCtx,
		&auth.GenerateAuthChallengeRequest{
			Role:   role,
	...
	respToken, err := as.AuthService.GenerateAuthTokens(as.GRPCCtx, &auth.GenerateAuthTokensRequest{
		Challenge:       challenge,
		SignedChallenge: sig,
	...
	go func() {
		for {
			...
			case <-as.GRPCCtx.Done():
				...
				return
			default:
				resp, err := as.AuthService.RefreshAccessToken(as.GRPCCtx, &auth.RefreshAccessTokenRequest{
...
func (as *AuthenticationService) updateAuthorizationMetadata(token *auth.Token) {
	as.mu.Lock()
	defer as.mu.Unlock()
